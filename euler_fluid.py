...
@ti.kernel
def v_visual(v_in:ti.template(), v: ti.template()):
    for i, j in v:
        norm=v_in[i,j][0]*v_in[i,j][0]+v_in[i,j][1]*v_in[i,j][1]
        # print(norm)
        v[i, j][0] = norm/(1+norm)
...
def main():
    ...
    while gui.running:
        ...
        v_f=ti.Vector.field(3,float,shape=(res,res))
        v_visual(velocities_pair.cur,v_f)
        gui.set_image(v_f)
